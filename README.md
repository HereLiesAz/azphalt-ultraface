...
**UltraFace (lightweight face detection)** — Fast face bounding-box detection.
...
This plugin contributes the role(s): `face-detection`. A host routes the model by role — it carries no
`targetApps`, so it is not tied to any single application.

**Example host — [Guillotine](https://github.com/HereLiesAz/Guillotine):** Desktop `faceDetectModelPath` — face blur.

## Model file(s)

- **`version-RFB-320.onnx`** (role `face-detection`) — [upstream](https://huggingface.co/onnx-community/ultraface-version-RFB-320/resolve/main/version-RFB-320.onnx)

Model license: **MIT (Ultra-Light-Fast-Generic-Face-Detector-1MB)**. This plugin's manifest/packaging is `MIT`.
...
```sh
npm install && npm run build     # packs com.hereliesaz.azphalt.ultraface-1.0.0.azp
